...
void run_test_3(void){
  int result = system("../main -d fichier0.txt fichier1.txt fichier2.txt fichierOut.txt");
...
int main(){
  ...
  pSuite = CU_add_suite("Test_3", setup, NULL);
  if (NULL == pSuite) {
    CU_cleanup_registry();
    return CU_get_error();
  }

  if ((NULL == CU_add_test(pSuite, "run_test_3", run_test_3)))
  {
    CU_cleanup_registry();
    return CU_get_error();
  }
