...
int setup(void){
  ...
  for(int k = 0; k < 156; k++){
    ...
    else sprintf(filename, "fract%d.bmp", k-7);
...
void run_test_4(void){
  int result = system("../main -d --maxthreads 10 fichier0.txt fichier1.txt fichier2.txt fichierOut.txt");
  CU_ASSERT_NOT_EQUAL(-1, result);
  const char *str = read_from_file("fichierOut.txt");
  CU_ASSERT_STRING_EQUAL(str, "fract34 500 500 0.360624 0.360624\n");
}
...
int main(){
  ...
  pSuite = CU_add_suite("Test_4", setup, NULL);
  if (NULL == pSuite) {
    CU_cleanup_registry();
    return CU_get_error();
  }

  if ((NULL == CU_add_test(pSuite, "run_test_4", run_test_4)))
  {
    CU_cleanup_registry();
    return CU_get_error();
  }
